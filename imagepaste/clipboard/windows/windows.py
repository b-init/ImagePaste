...
class WindowsClipboard(Clipboard):
    ...
    def push(cls, save_directory: str) -> WindowsClipboard:
        ...
        # Get image if available as 'PNG' or 'image/png' which covers pretty much all
        # software. Ditched BITMAP support because blender doesn't completely support
        # all Bitmap sub-formats and I couldn't find any software that copies only as a
        # bitmap.
    ...
    def pull(cls, image_path: str) -> WindowsClipboard:
        ...
        clipette.open_clipboard()
        clipette.empty_clipboard()

        image_path = abspath(image_path)
        image_format = image_path[-3:].lower()
        # bmp (as DIB, DIBV5, BITMAP) and png (as PNG) should be enough formats to work
    ...
    def convert_image(image_path: str, format: str) -> str:
        ...
        Args:
            ...
            format (str): Format to convert image to as in the image extension ('png',
                'bmp', etc)
        ...
        image_path_c = join(
            get_save_directory(), splitext(basename(image_path))[0] + format_ext[format]
        )
